...
\begin{table}[h]
    ...
    \begin{tabular}{p{0.44\textwidth}ccc}
        ...
        \hline
        \verb|valid_time=120, latitude=30,|\\ \verb|longitude=30| (Balanced) & 1485.06 & 582.7 & 258,188 \\
        \verb|valid_time=-1, latitude=20,|\\ \verb|longitude=20| (Space-First) & 2967.91 & $-500.4$ & 44,422 \\
        \verb|valid_time=24| (Time-First) & 522.90 & 82.8 & 2,870 \\
        \hline
    \end{tabular}
\end{table}
...
The results show a clear trade-off between how small or large the tasks are,
    ...
    Here, RAM increase is reported as a net process-level delta; negative values
    can occur when memory is released after compute.

\begin{itemize}
    \item \textbf{Time-First chunking (valid\_time=24)} is fastest in this run.
    It keeps scheduler overhead low while still exposing enough parallel work,
    yielding the shortest wall-clock time and lowest task count.

    \item \textbf{Balanced chunking} is slower than Time-First here, despite
    producing a very large task graph; the broader chunking across all axes
...
memory use. In this local run, Time-First with \verb|valid_time=24| gives the
best temporal-aggregation performance, while Space-First remains the slowest.
