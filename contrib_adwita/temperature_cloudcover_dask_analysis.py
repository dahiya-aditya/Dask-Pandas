...
if __name__ == "__main__":
    ...
    try:
        ...
        ds = xr.open_mfdataset(data_path, chunks={'valid_time': 24}, parallel=True, compat='no_conflicts')
        ...
        out = utils.save_figure(plt.gcf(), "t2m_global_vs_southasia", formats=["pdf"])
        ...
        plt.legend()
        plt.grid()
        out = utils.save_figure(plt.gcf(), "tcc_global_vs_southasia", formats=["pdf"])
        print(f"Saved figure: {out}.pdf")
    finally:
        plt.close("all")
        client.close()
